...
UKF::UKF() {
  ...
  */
  ...
  Xsig_pred_ = MatrixXd(n_x_, sigma_points);
...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  ...
  */
	...
	****************************************************************************/
	...
	const float dt = (meas_package.timestamp_ - previous_timestamp_) / 1000000.0;	//dt - expressed in seconds
	previous_timestamp_ = meas_package.timestamp_;
...
MatrixXd UKF::GenerateSigmaPoints() {
	...
	MatrixXd Xsig_aug = MatrixXd(n_aug_, sigma_points);
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
	...
	****************************************************************************/
	...
	MatrixXd Zsig = Xsig_pred_.block(0, 0, n_z, sigma_points);
	...
	****************************************************************************/
	...
	double nis;
	nis = Update(meas_package, n_z, Zsig, z_pred, S, z);
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  */
	...
	****************************************************************************/
	...
	MatrixXd Zsig = MatrixXd(n_z, sigma_points);
	...
	for (int i = 0; i < sigma_points; i++) {  //2n+1 simga points
		...
		if (fabs(rho) < 0.0001) {
			...
			rho_dot = (p_x*v1 + p_y*v2) / sqrt(p_x*p_x + p_y*p_y);
		}
...
/**
* Updates the state and the state covariance matrix common function
* @param n_z, z
*/
double UKF::Update(MeasurementPackage meas_package,
	int n_z,
	...
	double nis_ = (meas_package.raw_measurements_ - z_pred).transpose() * S.inverse() *
